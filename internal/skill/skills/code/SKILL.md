...
- File ops via read_file/write_file/edit_file/list_directory; search via glob/grep; web via web_fetch
...
- Prefer tools over bash for file reads/search (use read_file/glob/grep/list_directory).
- You can call multiple tools in a single turn; batch independent read_file/glob/grep/edit_file calls (use multi_edit for many precise edits).
...
- Analysis/Debug: locate key files with glob/grep, read_file with context, explain findings; batch inspections
